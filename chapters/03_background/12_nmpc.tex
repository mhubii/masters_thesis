...
\end{pmatrix},
\end{align}
and $\bm{B}_{k+1}=\begin{pmatrix} \bm{B}^{L/R} & \dots & \bm{B}^{L/R}\end{pmatrix}^T$. Whether the left foot's or the right foot's convex hull is chosen depends on the support foot at the respective preview interval $k$. Equation \ref{eq::312_cop_hull} can now be expressed in terms of the free variables $\bm{U}_k$ by
\begin{align}
	\bm{D}_{k+1}\begin{pmatrix}
		\bm{P}_{zu} & -\bm{V}_{k+1} & \bm{0} & \bm{0} & \bm{0} & \bm{0}\\
		\bm{0} & \bm{0} & \bm{P}_{zu} & -\bm{V}_{k+1} & \bm{0} & \bm{0}
	\end{pmatrix}\bm{U}_k \leq \begin{pmatrix}
	\bm{B}_{k+1}\\\bm{0}\\\bm{0}\end{pmatrix} + \begin{pmatrix}\bm{D}_{k+1}\\\bm{0}\\\bm{0}\end{pmatrix}\begin{pmatrix}
		-\bm{P}_{zs}\bm{c}_k^x + \bm{v}_{k+1}f_k^x\\
		-\bm{P}_{zs}\bm{c}_k^y + \bm{v}_{k+1}f_k^y\\\bm{0}\\\bm{0}
	\end{pmatrix}
\end{align}
...
1 \\ 0 \\ \vdots \\ 0
\end{pmatrix}\in\mathcal{R}^{N_f\times1}$. Altogether we have (\href{https://github.com/mhubii/nmpc_pattern_generator/blob/dc1f5a9366cbbbf76f1b02cada642f6ac9a04c89/libs/pattern_generator/src/base_generator.cpp#L1061}{link})
\begin{align}
\begin{pmatrix}
...
\end{pmatrix} &\leq \bm{B}_{k+1} \\
\begin{pmatrix}
\bm{0} & \bm{A}_{k+1}^{x}(\textbf{I}_{N_f}-\bm{S}_0) & \bm{0} & \bm{A}_{k+1}^{y}(\textbf{I}_{N_f}-\bm{S}_0) & \bm{0} & \bm{0}
\end{pmatrix}\bm{U}_k &\leq \begin{pmatrix}\bm{B}_{k+1}\\\bm{0}\\\bm{0}\end{pmatrix}+\begin{pmatrix}
\bm{S}_1f_k^x \\ \bm{S}_1f_k^x \\ \bm{0} \\ \bm{0} 
\end{pmatrix},
\end{align}
...
By considering the maximum and minimum angle by which the feet are relatively oriented towards each other, we take hardware limits into account. Furthermore, the restriction of the maximally allowed relative angular velocity decreases that variation of acceleration before the foot landing \cite{naveau2016reactive}. The constraints can be formulated as follows (\href{https://github.com/mhubii/nmpc_pattern_generator/blob/dc1f5a9366cbbbf76f1b02cada642f6ac9a04c89/libs/pattern_generator/src/base_generator.cpp#L1244}{link})
\begin{align}
...
\begin{align}
	...
	\bm{0} & \bm{0} &\bm{0} &\bm{0} & \bm{P}_{pu} & -\bm{P}_{pu}
	\end{pmatrix}\bm{U}_k \leq \bm{\theta}_\text{max}-\bm{P}_{ps}(f_k^L-f_k^R) \\
		-\dot{\bm{\theta}}_\text{max}-\bm{P}_{vs}(f_k^L-f_k^R)\leq \begin{pmatrix}
	\bm{0} & \bm{0} &\bm{0} &\bm{0} & \bm{P}_{vu} & -\bm{P}_{vu}
	\end{pmatrix}\bm{U}_k \leq \dot{\bm{\theta}}_\text{max}-\bm{P}_{vs}(f_k^L-f_k^R).
\end{align}
\paragraph{Obstacle Constraints}
In contrast to similar methods like \cite{herdt2010walking}, we also include the avoidance of convex obstacles by requesting that the feet's positions must lie outside of circles $C = \{(p^x, p^y)\in\mathbb{R}^2 \,|\, (p^x-x_0)^2+(p^y-y_0)^2\leq R^2\}$, which define the obstacles, where $x_0$, and $y_0$ define the obstacle's center in world coordinates. This can be formulated by the free variables by (\href{https://github.com/mhubii/nmpc_pattern_generator/blob/dc1f5a9366cbbbf76f1b02cada642f6ac9a04c89/libs/pattern_generator/src/base_generator.cpp#L1271}{link})
\begin{align}
	&(f_k^x-x_0)^2 + (f_k^y-y_0)^2 \geq (R + m)^2\\
	&\bm{U}_k^T\begin{pmatrix}
	\bm{0} & \textbf{I}_{N_f} & \bm{0}& \textbf{I}_{N_f} & \bm{0} & \bm{0}
	\end{pmatrix}\bm{U}_k + \\ &\begin{pmatrix}
	\bm{0} & \begin{pmatrix}
	-2x_0 & \dots & -2x_0
	\end{pmatrix} & \bm{0} & \begin{pmatrix}
	-2y_0 & \dots & -2y_0
	\end{pmatrix} & \bm{0} & \bm{0}
